    ...
    <div className="glass rounded-4xl p-6 shadow-premium transition-all hover:shadow-lg">
      <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
        <div className="flex gap-4">
          ...
          <div>
            <div className="text-xl font-bold text-foreground">{doctor.name}</div>
    ...
    >
      ...
      <RequireRole role="admin">
          ...
          <div className="mb-8 glass rounded-4xl p-6 sm:p-8 shadow-premium border-2 border-primary/20">
            ...
            <div className="mt-8 pt-8 border-t border-foreground/5">
              <div className="flex items-center justify-between mb-4">
                <div className="text-sm font-bold text-foreground">Consultation Types & Pricing</div>
                <button
                  onClick={() => setConsults((cs) => [...cs, { id: uid("c"), title: "", price: 0 }])}
              ...
              <div className="grid gap-3">
                {consults.map((c, idx) => (
                  ...
                  >
                    <input
                      value={c.title}
                      onChange={(e) => setConsults((cs) => cs.map((x) => x.id === c.id ? { ...x, title: e.target.value } : x))}
                    ...
                    <div className="flex gap-3 w-full sm:w-auto">
                      <div className="relative flex-1 sm:w-32">
                        ...
                        <input
                          value={c.price}
                          onChange={(e) => setConsults((cs) => cs.map((x) => x.id === c.id ? { ...x, price: Number(e.target.value || 0) } : x))}
                          type="number"
                          min={0}
                      ...
                      <button
                        onClick={() => setConsults((cs) => cs.filter((x) => x.id !== c.id))}
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3">
              ...
              <button
                onClick={() => { setMode(null); if (editingDoctor) startEdit(editingDoctor); }}
        ...
        <div className="grid gap-6 mt-6">
          {doctors.map((d) => (
            <DoctorCard key={d.id} doctor={d} onEdit={() => startEdit(d)} onDelete={() => removeDoctor(d.id)} />
          ))}
